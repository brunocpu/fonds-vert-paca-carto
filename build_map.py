...
OUTPUT_HTML = Path("fonds-vert-paca-carte.html")
...
LINE_CORRECTIONS = {
    ...
    # Intra PACA (siège bénéficiaire -> lieu projet)
    ...
    8276: '04049',  # Marseille -> Château-Arnoux (article mesinfos.fr, Résidence LAPIE)
    8661: '83098',  # Marseille -> Le Pradet (site MFP, 38 logements BRS/locatif)
    # --- 2 approximations ---
    8776: '84031',  # Aix -> Carpentras approx (CITTA, étude en Vaucluse)
    8553: '84138',  # Nyons (26) -> Valréas (SM Eygues, bassin versant 26/84)
    # --- 7 PAPI Côtiers des Maures (Canal de Provence siège Le Tholonet -> Le Muy) ---
    8679: '83086', 8680: '83086', 8682: '83086', 8683: '83086',
    8685: '83086', 8686: '83086', 8690: '83086', 8695: '83086',
}

EXCLUDE_LINES = set()  # Zéro exclusion
...
DEMARCHE_SHORT = {
    ...
    "Prévention des risques d'incendies de forêt et de végétation": "Incendies forêt",
...
def download_csv():
    if CSV_PATH.exists():
        print(f"CSV present: {CSV_PATH}")
        return
    print(f"Downloading CSV...")
    CSV_PATH.parent.mkdir(parents=True, exist_ok=True)
    urllib.request.urlretrieve(CSV_URL, CSV_PATH)
    print(f"Downloaded: {CSV_PATH} ({CSV_PATH.stat().st_size / 1e6:.1f} MB)")
...
def parse_csv():
    ...
    with open(CSV_PATH, 'r', encoding='utf-8') as f:
        ...
        for i, row in enumerate(reader):
            ...
            # Apply corrections (both hors-PACA and intra-PACA)
            if line in LINE_CORRECTIONS:
                code = LINE_CORRECTIONS[line]
                stats['corrected'] += 1
            elif code[:2] not in PACA_DEPTS:
                # Unknown hors-PACA, skip (should not happen if corrections are complete)
                continue

            try:
                m = row.get('montant_engage', '0').replace('\xa0', '').replace(' ', '').replace(',', '.')
...
def geocode(communes):
    ...
    for i, code in enumerate(codes):
        ...
        if (i + 1) % 50 == 0:
            print(f"  Geocoded {i + 1}/{len(codes)}")
    print(f"Geocoded: {len(geocoded)}/{len(codes)}")
    return geocoded
